...
#include "src/INT4BF16LinearAdd.hpp"

namespace py = pybind11;

struct INT4BF16LinearAdd{
    int input_dim;
    int output_dim;
    int group_size;
    INT4BF16Add kernel;

    INT4BF16LinearAdd(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4BF16LinearAdd, m) {
    py::class_<INT4BF16LinearAdd>(m, "INT4BF16LinearAdd")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4BF16LinearAdd::kernel)
        .def_readwrite("input_dim", &INT4BF16LinearAdd::input_dim)
        .def_readwrite("output_dim", &INT4BF16LinearAdd::output_dim)
        .def_readwrite("group_size", &INT4BF16LinearAdd::group_size)
        .def("forward", &INT4BF16LinearAdd::forward);
};