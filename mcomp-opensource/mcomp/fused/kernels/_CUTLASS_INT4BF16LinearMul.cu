...
#include "src/INT4BF16LinearMul.hpp"

namespace py = pybind11;

struct INT4BF16LinearMul{
    int input_dim;
    int output_dim;
    int group_size;
    INT4BF16Mul kernel;

    INT4BF16LinearMul(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4BF16LinearMul, m) {
    py::class_<INT4BF16LinearMul>(m, "INT4BF16LinearMul")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4BF16LinearMul::kernel)
        .def_readwrite("input_dim", &INT4BF16LinearMul::input_dim)
        .def_readwrite("output_dim", &INT4BF16LinearMul::output_dim)
        .def_readwrite("group_size", &INT4BF16LinearMul::group_size)
        .def("forward", &INT4BF16LinearMul::forward);
};