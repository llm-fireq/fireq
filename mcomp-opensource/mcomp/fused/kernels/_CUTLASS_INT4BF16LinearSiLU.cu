...
#include "src/INT4BF16LinearSiLU.hpp"

namespace py = pybind11;

struct INT4BF16LinearSiLU{
    int input_dim;
    int output_dim;
    int group_size;
    INT4BF16SiLU kernel;

    INT4BF16LinearSiLU(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4BF16LinearSiLU, m) {
    py::class_<INT4BF16LinearSiLU>(m, "INT4BF16LinearSiLU")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4BF16LinearSiLU::kernel)
        .def_readwrite("input_dim", &INT4BF16LinearSiLU::input_dim)
        .def_readwrite("output_dim", &INT4BF16LinearSiLU::output_dim)
        .def_readwrite("group_size", &INT4BF16LinearSiLU::group_size)
        .def("forward", &INT4BF16LinearSiLU::forward);
};