...
#include "src/INT4BF16Linear.hpp"

namespace py = pybind11;

struct INT4BF16Linear{
    int input_dim;
    int output_dim;
    int group_size;
    INT4BF16 kernel;

    INT4BF16Linear(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4BF16Linear, m) {
    py::class_<INT4BF16Linear>(m, "INT4BF16Linear")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4BF16Linear::kernel)
        .def_readwrite("input_dim", &INT4BF16Linear::input_dim)
        .def_readwrite("output_dim", &INT4BF16Linear::output_dim)
        .def_readwrite("group_size", &INT4BF16Linear::group_size)
        .def("forward", &INT4BF16Linear::forward);
};