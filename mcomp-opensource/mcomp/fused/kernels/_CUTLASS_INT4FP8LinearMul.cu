...
#include "src/INT4FP8LinearMul.hpp"

namespace py = pybind11;

struct INT4FP8LinearMul{
    int input_dim;
    int output_dim;
    int group_size;
    INT4FP8Mul kernel;

    INT4FP8LinearMul(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4FP8LinearMul, m) {
    py::class_<INT4FP8LinearMul>(m, "INT4FP8LinearMul")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4FP8LinearMul::kernel)
        .def_readwrite("input_dim", &INT4FP8LinearMul::input_dim)
        .def_readwrite("output_dim", &INT4FP8LinearMul::output_dim)
        .def_readwrite("group_size", &INT4FP8LinearMul::group_size)
        .def("forward", &INT4FP8LinearMul::forward);
};