...
#include "src/INT4FP8Linear.hpp"

namespace py = pybind11;

struct INT4FP8Linear{
    int input_dim;
    int output_dim;
    int group_size;
    INT4FP8 kernel;

    INT4FP8Linear(torch::Tensor weights, 
...
PYBIND11_MODULE(_CUTLASS_INT4FP8Linear, m) {
    py::class_<INT4FP8Linear>(m, "INT4FP8Linear")
        .def(py::init<torch::Tensor, torch::Tensor, int, int, int>())
        .def_readwrite("kernel", &INT4FP8Linear::kernel)
        .def_readwrite("input_dim", &INT4FP8Linear::input_dim)
        .def_readwrite("output_dim", &INT4FP8Linear::output_dim)
        .def_readwrite("group_size", &INT4FP8Linear::group_size)
        .def("forward", &INT4FP8Linear::forward);
};